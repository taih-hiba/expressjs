...
var FindAndGo = L.Class.extend({
    ...
    options: {
        ...
        locateZoom : 6, // Min zoom level when locating an address
    ...
    _loadPanel : function(divId) {
        ...
        request.open("GET", "http://localhost:3000/dom.html", true);
    ...
    addressFound : function(geoJsonData) {
        ...
        if (geoJsonData.features.length === 0) {
            this.showAddress("no adresse trouvee");
        } else {
    ...
    _displayOverpassResponse : function(data, layerInfo, distance) {
        ...
        if (numItems > 0) {
            var str = (numItems === 1) ? " �l�ment" : " �l�ments";
            this.feedbackSearch("Trouv� " + numItems + str + " dans OpenStreetMap");
            var layer = this.buildLayer(data.elements, layerInfo);
            this.addLayer(layer, layerInfo);
            this.stepReady("FaG-step3");
        } else {
            this.feedbackSearch("Aucun �l�ment trouv� dans un rayon de " + distance);
        }
    },
    ...
    destinationFound: function(geoJsonData) {
        ...
        if (geoJsonData.features.length === 0) {
            addressItem.innerHTML = "Adresse non identifi�e ...";
        } else {
            var address = geoJsonData.features[0].properties.label;
    ...
    requestRoute: function(vehicle) {
        ...
        request.onload = function() {
            if (this.status === 200) {
                ...
                _this.feedbackRoute("le calcul d'itin�raire n'a pas abouti.");
            }
        };
        request.send();
        this.feedbackRoute("Calcul d'itin�raire en cours ...");
    },
    ...
    displayRoute: function(result, vehicle) {
        if (result.paths) {
            ...
            var typeDesc = (vehicle === 'car') ? 'en voiture' : 
                    (vehicle === 'bike') ? 'en v�lo' : '� pied';
            this.feedbackRoute("Distance " + typeDesc + " : " + 
                    distance + " (dur�e : " + duration +")");
